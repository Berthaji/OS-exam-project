...
void *pEnemy1(void *obj)
{
    ...
    while (loop)
    {
        ...
        if (enemies1[k].y >= SCREEN_H - 2)
        { //Bordo inferiore superato
            enemies1[k].y--;
            enemies1[k].dir = 1;
        }
        if (enemies1[k].y <= 0)
        { //Bordo superiore superato
            enemies1[k].y++;
            enemies1[k].dir = 0;
        }
...
void tEngine(int life)
{
    ...
    for (int i = 0, x = 0, y = 1; i < enemies1Count; i++)
    {
        ...
        if (pthread_create(&t_enemy1, NULL, pEnemy1, (void *)i))
        { //passo i camuffato da void al thread
            endwin();
            exit(-1);
        }
    }
    ...
    while (loop)
    {
        ...
        { //Lancio dei due missili
            ...
            { //Aggiunta dei due missili all'elenco
        ...
        //ENEMY1
        for (int k = 0; k < enemies1Count; k++)
        { //scorre ogni nemico di primo livello della run
            ...
            if (match > -1)
            { //Match nemico astronave
                life--;

                //Ammazzo il nemico colpito
                kill(enemies1[k].pid, SIGKILL);
                enemies1[match].y = 0;
                enemies1[match].x = 0;
                enemies1[match].state = DEAD;

                //Ammazzo l'astronave stessa
                if (astroship.state != DEAD)
                { //Ammazzo astronave solo se viene colpito una volta
                    //Ammazzo l'astronave stessa
        ...
        for (int k = 0; i < missilesCount; i++)
        {
            ...
            int enemy2id = MissileCollided(enemies2, missiles[k], enemies2Count);
            if (enemy2id > -1)
            { //Match nemico2 missile
                if (enemies2[enemy2id].state != DEAD)
                { //Ammazzo il nemico solo se viene colpito una volta (ossia dal primo missile)
                    //Ammazzo il missile stesso
                    kill(missiles[k].pid, SIGKILL); //k??
                    missiles[k].y = -1;
                    missiles[k].x = -1;
                    missiles[k].state = DEAD;
        ...
        for (int k = 0; i < missilesCount; i++)
        {
            ...
            //Controllo dell'incrocio con l'oggetto astronave
            int match = AstroCollided(astroship, enemies2[k]);
            if (match > -1)
            { //Match nemico astronave
                life--;

                //Kill del nemico stesso
                kill(enemies2[match].pid, SIGKILL);
                enemies2[match].y = -5;
                enemies2[match].x = 0;
                enemies2[match].state = DEAD;
