...
int main()
{
	...
	pidA=fork();
		...
		/*Sembra succeda qualcosa di strano per cui 
		  non entra nel processo che ha pidG, perchè
		  la printf() non restituisce nessun pid,
		  quindi quell'istruzione non viene mai letta.
		  Non è certo che la funzione area() sia giusta*/
		if(pidA==0){
			mvprintw(0,MAXY/2,"+");
			close(p[0]);
			astro(p[1]);
